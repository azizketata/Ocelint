...
class Config:
    ...
    expected_types: dict[str, list[str]] = field(default_factory=dict)
...
def load_config(path: Path | None = None) -> Config:
    ...
    return Config(
        ...
        expected_types=_expected_types_map(section.get("expected-types", {}), target),
    )
...
def _expected_types_map(value: object, path: Path) -> dict[str, list[str]]:
    if not isinstance(value, dict):
        raise ConfigError(f"{path}: [tool.ocelint.expected-types] must be a table")
    out: dict[str, list[str]] = {}
    for etype, otypes in value.items():
        if not isinstance(etype, str):
            raise ConfigError(f"{path}: expected-types keys must be strings")
        if not isinstance(otypes, list) or not all(isinstance(o, str) for o in otypes):
            raise ConfigError(
                f"{path}: expected-types[{etype!r}] must be a list of object-type strings"
            )
        out[etype] = list(otypes)
    return out


def render_init_template(rules: list[Rule]) -> str:
    """Render a [tool.ocelint] block listing all rules with default severities."""
    lines = [
        "# Generated by `ocelint init`. Edit to customize.",
        "[tool.ocelint]",
        '# select = ["S", "R001"]      # only S-rules and R001',
        '# ignore = ["S009"]            # disable a specific rule',
        '# extend-ignore = ["R005"]     # extend the default ignore set',
        "",
        "# Per-rule severity overrides. Defaults listed below; uncomment to change.",
        "[tool.ocelint.severity]",
    ]
    for rule in rules:
        lines.append(f'# {rule.code} = "{rule.severity}"  # {rule.description}')
    lines.extend([
        "",
        "# Expected object types per event type (powers R008).",
...
__all__ = ["Config", "ConfigError", "filter_rules", "load_config", "render_init_template"]
